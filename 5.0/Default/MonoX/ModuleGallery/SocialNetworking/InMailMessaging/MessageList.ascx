        ...
        <ItemTemplate>
            <div class="message-list <%# (Container.DataItemIndex + 1) % 2 == 0 ? "even" : "odd" %> <%# (Container.DataItemIndex) == 0 ? "first" : "" %> <%# ((SnMessageEntity)Container.DataItem).IsRead(MonoSoftware.MonoX.Utilities.SecurityUtility.GetUserId()) ? "message-read" : "message-unread"  %>" >
                <div class="message-holder" >
                    ...
                    <div class="date"><%# DependencyInjectionFactory.Resolve<MonoSoftware.MonoX.Repositories.IUserRepository>().ConvertTimeFromUtcToUserLocalTime(MonoSoftware.MonoX.Utilities.SecurityUtility.GetUserId(), ((SnMessageEntity)Container.DataItem).DateCreated)%></div>
