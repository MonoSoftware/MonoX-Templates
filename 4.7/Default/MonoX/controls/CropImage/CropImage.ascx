...
<script type="text/javascript">
    ...
    function setupCrop(){        
        $('.image-crop-holder').Jcrop({
            ...
            boxWidth: <%= this.BoxWidth %>, boxHeight : <%= this.BoxHeight %>, minSize: [<%= this.CropMinSizeWidth %>,<%= this.CropMinSizeHeight %>], parentDiv: "#cropHolder"
        });        
    };
    ...
    function storeCoords(c){
        ...
        jQuery('#<%= imgCropH.ClientID %>').val(parseInt(c.w));
        jQuery('#<%= imgCropW.ClientID %>').val(parseInt(c.h));
    };    
     //]]>    
</script>
